...
namespace ShellLauncher
{
    public class AppConfig : System.ComponentModel.INotifyPropertyChanged
    {
        ...
        public string? Name  { get => _name;  set { _name  = value; Notify(nameof(Name));  } }

        private string? _path;
        public string? Path  { get => _path;  set { _path  = value; Notify(nameof(Path));  } }

        private string? _args;
        public string? Args  { get => _args;  set { _args  = value; Notify(nameof(Args));  } }

        private bool _excludeFromTaskbar;
        public bool ExcludeFromTaskbar
        {
            get => _excludeFromTaskbar;
            set { _excludeFromTaskbar = value; Notify(nameof(ExcludeFromTaskbar)); }
        }
        ...
        public string? DependsOn
        {
            ...
            {
                _dependsOn = value;
                Notify(nameof(DependsOn));
                Notify(nameof(LaunchModeEnabled));
                // dependent apps must always use Monitor
                if (!string.IsNullOrWhiteSpace(value))
                    LaunchMode = "Monitor";
            }
        }

        private string _launchMode = "Monitor";
        public string LaunchMode
        {
            get => _launchMode;
            set
            {
                _launchMode = string.IsNullOrWhiteSpace(value) ? "Monitor" : value;
                Notify(nameof(LaunchMode));
                Notify(nameof(ExcludeFromTaskbarEnabled));
                // NoLaunch apps must always show in taskbar
                if (_launchMode == "NoLaunch")
                    ExcludeFromTaskbar = false;
            }
        }

        [System.Text.Json.Serialization.JsonIgnore]
        public bool RunOnce => LaunchMode == "RunOnce";

        // UI binding: ComboBox disabled when DependsOn is set
        [System.Text.Json.Serialization.JsonIgnore]
        public bool LaunchModeEnabled => string.IsNullOrWhiteSpace(DependsOn);

        // UI binding: CheckBox disabled when LaunchMode is NoLaunch
        [System.Text.Json.Serialization.JsonIgnore]
        public bool ExcludeFromTaskbarEnabled => LaunchMode != "NoLaunch";
    }
    ...
    class Program
    {
        ...
        static void NormalizeAppConfig(AppConfig app)
        {
            // NoLaunch must always appear in taskbar
            if (app.LaunchMode == "NoLaunch")
                app.ExcludeFromTaskbar = false;

            // DependsOn apps cannot have a custom LaunchMode — always Monitor
            if (!string.IsNullOrWhiteSpace(app.DependsOn))
                app.LaunchMode = "Monitor";
        }

        static List<AppConfig> LoadAndNormalize(string configPath)
        {
            var list = JsonSerializer.Deserialize<List<AppConfig>>(
                File.ReadAllText(configPath)) ?? new List<AppConfig>();
            foreach (var app in list) NormalizeAppConfig(app);
            return list;
        }
